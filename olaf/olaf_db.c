...
Olaf_DB * olaf_db_new(const char * db_file_folder, bool readonly)
{
  ...
  fprintf(
    stderr, "Mem DB for '%s', %.3fs duration, %zu fingerprints, %d identifier\n",
    olaf_db_mem_audio_path, olaf_db_mem_audio_duration, olaf_db_mem_fp_length,
...
size_t olaf_db_find(
  Olaf_DB * olaf_db, uint64_t start_key, uint64_t stop_key, uint64_t * results, size_t results_size)
{
  ...
  for (uint64_t current_key = start_key; current_key <= stop_key; current_key++) {
    ...
    match = (uint64_t *)bsearch(
      &packed_key, olaf_db->ref_fp, olaf_db->ref_fp_length, sizeof(uint64_t),
      olaf_dp_packed_hash_compare);
...
void olaf_db_destroy(Olaf_DB * olaf_db) { free(olaf_db); }
