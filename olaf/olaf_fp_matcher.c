...
void olaf_hash_table_value_free_func(void * value) { free(value); }

//Creates a new matcher
Olaf_FP_Matcher * olaf_fp_matcher_new(
  Olaf_Config * config, Olaf_DB * db, Olaf_FP_Matcher_Result_Callback callback)
{
  ...
  hash_table_register_free_functions(
    fp_matcher->result_hash_table, NULL, olaf_hash_table_value_free_func);
...
void olaf_fp_matcher_remove_old_matches(Olaf_FP_Matcher * fp_matcher, int current_query_time)
{
  ...
  int max_age = (int)((fp_matcher->config->keepMatchesFor * fp_matcher->config->audioSampleRate) /
                      fp_matcher->config->audioStepSize);
...
//
void olaf_fp_matcher_tally_results(
  Olaf_FP_Matcher * fp_matcher, int queryFingerprintT1, int referenceFingerprintT1,
  uint32_t matchIdentifier)
{
  ...
  struct match_result * match =
    (struct match_result *)hash_table_lookup(fp_matcher->result_hash_table, &result_hash_table_key);
  ...
  if (match != NULL) {
    ...
    match->firstReferenceFingerprintT1 =
      min(referenceFingerprintT1, match->firstReferenceFingerprintT1);
    match->lastReferenceFingerprintT1 =
      max(referenceFingerprintT1, match->lastReferenceFingerprintT1);
...
void olaf_fp_matcher_match_single_fingerprint(
  Olaf_FP_Matcher * fp_matcher, uint32_t queryFingerprintT1, uint64_t queryFingerprintHash)
{
  int range = fp_matcher->config->searchRange;
  size_t number_of_results = olaf_db_find(
    fp_matcher->db, queryFingerprintHash - range, queryFingerprintHash + range,
    fp_matcher->db_results, fp_matcher->config->maxDBCollisions);
  ...
  if (fp_matcher->config->verbose) {
    fprintf(
      ...
      fp_matcher->config->maxDBCollisions);
  }
  ...
  for (size_t i = 0; i < number_of_results && i < fp_matcher->config->maxDBCollisions; i++) {
    ...
    if (fp_matcher->config->verbose) {
      ...
      fprintf(
        stderr, "\taudio id: %u\n\tref t1: %u \n\tdelta qt1-ft1: %d \n", matchIdentifier,
        referenceFingerprintT1, delta);
    }

    olaf_fp_matcher_tally_results(
      fp_matcher, queryFingerprintT1, referenceFingerprintT1, matchIdentifier);
  }
}

void olaf_fp_matcher_match(
  Olaf_FP_Matcher * fp_matcher, struct extracted_fingerprints * fingerprints)
{
  ...
  if (fingerprints->fingerprintIndex > 0 && fp_matcher->config->printResultEvery != 0) {
    int printResultEvery =
      (fp_matcher->config->printResultEvery * fp_matcher->config->audioSampleRate) /
      fp_matcher->config->audioStepSize;
    int current_query_time =
      fingerprints->fingerprints[fingerprints->fingerprintIndex - 1].timeIndex3;
...
void olaf_fp_matcher_callback_print_result(
  int matchCount, float queryStart, float queryStop, const char * path, uint32_t matchIdentifier,
  float referenceStart, float referenceStop)
{
  printf(
    "%d, %.2f, %.2f, %s, %u, %.2f, %.2f\n", matchCount, queryStart, queryStop, path,
    matchIdentifier, referenceStart, referenceStop);
}
...
void olaf_fp_matcher_print_results(Olaf_FP_Matcher * fp_matcher)
{
  ...
  struct match_result ** match_results =
    (struct match_result **)calloc(match_results_max, sizeof(struct match_result *));
  ...
  while (hash_table_iter_has_more(&iterator)) {
    ...
    if (match->matchCount >= fp_matcher->config->minMatchCount) {
      if (match_results_max == match_results_index) {
        qsort(
          match_results, match_results_index, sizeof(struct match_result *),
          olaf_fp_sort_results_by_match_count);
  ...
  for (size_t i = 0; i < match_results_index; i++) {
    ...
    float secondsPerBlock =
      ((float)fp_matcher->config->audioStepSize) / ((float)fp_matcher->config->audioSampleRate);
    ...
    if ((referenceStop - referenceStart) >= fp_matcher->config->minMatchTimeDiff) {
      ...
      fp_matcher->result_callback(
        match->matchCount, queryStart, queryStop, meta_data.path, matchIdentifier, referenceStart,
        referenceStop);
    }
  }
