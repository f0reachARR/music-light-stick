...
Olaf_EP_Extractor * olaf_ep_extractor_new(Olaf_Config * config)
{
  ...
  ep_extractor->eventPoints.eventPoints =
    (struct eventpoint *)calloc(config->maxEventPoints, sizeof(struct eventpoint));
  ep_extractor->eventPoints.eventPointIndex = 0;
  if (ep_extractor->eventPoints.eventPoints == NULL)
    fprintf(stdout, "Failed to allocate memory: eventPoints");
...
void olaf_ep_extractor_max_filter_frequency(
  float * data, float * max, int length, int half_filter_size)
{
  ...
  for (size_t j = minFreqencyBin; j < halfAudioBlockSize - 1; j++) {
    ...
    if (currentVal == maxVal) {
      ...
      if (eventPointIndex == ep_extractor->config->maxEventPoints) {
        fprintf(
          stderr,
          "Warning: Eventpoint maximum index %d reached, event points are ignored, consider "
          "increasing config->maxEventPoints if you see this often. \n",
...
struct extracted_event_points * olaf_ep_extractor_extract(
  Olaf_EP_Extractor * ep_extractor, float * fft_out, int audioBlockIndex)
{
  ...
  for (int j = 0; j < ep_extractor->config->audioBlockSize; j += 2) {
    if (ep_extractor->config->sqrtMagnitude) {
      ep_extractor->mags[filterIndex][magnitudeIndex] =
        sqrt(fft_out[j] * fft_out[j] + fft_out[j + 1] * fft_out[j + 1]);
    } else {
      ep_extractor->mags[filterIndex][magnitudeIndex] =
  ...
  olaf_ep_extractor_max_filter_frequency(
    ep_extractor->mags[filterIndex], ep_extractor->maxes[filterIndex],
    ep_extractor->config->audioBlockSize / 2, ep_extractor->config->halfFilterSizeFrequency);
